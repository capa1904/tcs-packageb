...
let testProjectFile = "PackageB.Test.Unit/PackageB.Test.Unit.csproj"
// -------------------------------------------------------------------------------------------------------------------------------------------------

let projectFile = "PackageB/PackageB.csproj"
let packageName = "PackageB"
let nuspecFile = "PackageB.nuspec"
...
Target "BuildNuGetPackage" (fun _ ->
    ...
    CopyFiles (nugetInputFilesDir @@ "lib" @@ "net45") !!(buildDir + "PackageB.*")
    ...
    !!(testBuildDir @@ "**/*.Test.Unit.dll")
      |> NUnit (fun p ->
          {p with
             ...
             ShowLabels = false
             OutputFile = testBuildDir @@ "UnitTestResults_PackageB.xml" })
)
